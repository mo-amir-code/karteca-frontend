...
export const checkIsSelected = ({list, value}:{list: [ListType], value:string}) => {
...
{isSort:boolean, setSelected:Function, selected:ListType | ListType[], target:ListType}

export const handleSelectUtil = ({isSort, setSelected, selected, target}:HandleSelectUtilType) => {
    if (isSort) {
        setSelected(target);
      } else {
