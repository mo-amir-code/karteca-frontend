...
const Buttons = ({cartId, isOnMobile, productId}:{cartId:string, isOnMobile?:boolean, productId: string}) => {
  ...
  return (
    <>
      <button onClick={()=>handleDeleteCartItem()} className={`flex smooth_transition items-center justify-center gap-1 text-text-color bg-primary-color ${isOnMobile? "p-1 text-xs rounded-sm" : "text-sm hover:-translate-y-1 px-4 p-2 rounded-lg shadow-lg" } smooth_transition`}>
        <FaTrash size={12} />
        <span>Remove</span>
      </button>
