...
const Category = ({name, image}:ProductCategoryWithImageType) => {
  return (
    <Link href={`/search?category=${name.toLowerCase()}`} className="flex cursor-pointer flex-col gap-1 items-center justify-center">
          ...
          <span className="max-sm:text-sm max-[550px]:text-[10px]" >{name}</span>
    </Link>
  )
}
